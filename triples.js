...
function Triples(database) {
  ...
  async function put(subject, predicate, object) {
    ...
    let write = await database.importDB([spo,sop,ops,osp,pso,pos]);
    return true;
  ...
  async function del(subject, predicate, object) {
    ...
    let remove = await database.del([spo,sop,ops,osp,pso,pos]);
    return true;
  ...
  async function get(subject, predicate, object) {
    ...
    let exists = await database.get(key);
    if (exists.value) {
  ...
  async function list(subject, predicate, object) {
    ...
    if (search.length === 3) {
      let result = await database.get(q.slice(0, -2));
